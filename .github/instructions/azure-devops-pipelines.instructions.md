...
## Acidni-Specific Pipeline Standards

When creating pipelines for Terprint/Acidni projects:

- **Service Connections**: Use managed identity-based connections where possible
- **Environments**: Use `dev`, `staging`, `prod` naming convention
- **Approval Gates**: Require manual approval for production deployments
- **Artifacts Feed**: Publish Python packages to `terprint` Azure Artifacts feed
- **Test Integration**: Trigger `Terprint.Tests` pipeline after component deployments
- **Key Vault**: Reference secrets from `acidni-keyvault` or `kv-terprint`
