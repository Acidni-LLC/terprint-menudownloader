...
async def get_dispensary_stock(
    ...
    logger.info(f"🔥 GET /{dispensary} endpoint called (category={category}, limit={limit})")
    index = get_stock_index()
...
async def build_stock_index(
    ...
    logger.info(f"🔥 POST /build-index endpoint called with date={date}")
    try:
        indexer = get_indexer()
...
# === CATCH-ALL PARAMETER ROUTES (MUST BE LAST) ===

@router.post("/bulk-check")
async def bulk_stock_check(request: BulkStockRequest):
    """Check stock for multiple batches at once (accepts batch_ids array in body)."""
    index = get_stock_index()
    
    if not request.batch_ids:
        raise HTTPException(
            status_code=400,
            detail="batch_ids array cannot be empty"
        )
    
    results = []
    
    for batch_id in request.batch_ids:
        # Search across all dispensaries for this batch
        found_item = None
        for dispensary_items in index["by_dispensary"].values():
            for item in dispensary_items:
        ...
        if found_item:
            results.append({
                "batch_id": batch_id,
                "found": True,
                "dispensary": found_item.get("dispensary"),
                "item": found_item
            })
        else:
            results.append({
