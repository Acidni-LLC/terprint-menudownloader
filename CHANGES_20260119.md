...
1. **âœ… Batch Creator Integration** - Menu downloads now trigger the Batch Creator service
2. **âœ… OpenAPI Specification** - Created complete API spec for APIM import
3. **âœ… Documentation Updates** - Updated README with 5-stage pipeline flow
4. **âœ… Flowery Downloads** - Fixed Flowery Bearer token, now downloading 16 locations
5. **âœ… Stock Index Auto-build** - Automatically rebuilds after batch creation
...
```python
BATCH_CREATOR_URL = "https://ca-terprint-batches.greenbay-731aa80e.eastus2.azurecontainerapps.io/api/create-batches"
```
...
- Shows data flow from downloads â†’ batch creation â†’ COA processing â†’ stock index
...
- Status endpoint: âœ… Version 2.0.0
- Stock API: âœ… Returning 495 unique strains
...
2. **Test Stock Endpoints Through APIM**
   ...
   # Test stock search
   Invoke-RestMethod -Uri "https://api.acidni.net/menus/api/stock/search?strain=Gelato" `
...
| Service | URL | Status |
|---------|-----|--------|
| Menu Downloader | `ca-terprint-menudownloader.greenbay-731aa80e.eastus2...` | âœ… Running |
| Batch Creator | `ca-terprint-batches.greenbay-731aa80e.eastus2...` | âœ… Running |
| Batch Processor | `ca-terprint-batchprocessor.greenbay-731aa80e.eastus2...` | âœ… Running |
| APIM Gateway | `api.acidni.net` | â³ Needs stock endpoints |
| Storage Account | `stterprintsharedgen2/jsonfiles` | âœ… Storing data |
